...
def zmq_worker(ai: AzimuthalIntegrator, 
    ...
    while True:
        ...
        if header['htype'] == 'image':
            ...
            if mask_threshold:
                mask_dtype = get_mask_dtype(img)
                mask = np.zeros(img.shape, dtype=mask_dtype)
...
def test_worker(ai: AzimuthalIntegrator,
                filename: str,
                dset_name: str,
                collector_port: int,
                worker_id: int, 
    ...
    for i in range(worker_id, nimages, nworkers):
        data = {}
        data['/entry/data1d/I'] = np.zeros((100,))
...
class Pipeline():
    ...
    def start(self, config, mode, nworkers, host=None, 
        ...
        for i in range(nworkers):
            if mode == 'file':
                p = Process(target=test_worker, args=(self.ai, filename, dset, self.collector_port, i, nworkers, mask_threshold))
            else:
    ...
    def write_header(self):
        ...
        poni_file = self.config['poni']
        with open(poni_file, 'r') as pf:
            dset = parameters.create_dataset('poni', data=pf.read())
...
def make_app(pipeline, args):
    ...
    async def start(request: Request):
        ...
        app.state.config = config.copy()
        
        mask_threshold = config.pop('mask_threshold')
        print(config)
        pipeline.start(config, 'stream', args['nworkers'], args['host'], args['data_port'], mask_threshold=mask_threshold)
        return {'value': 'done'}
